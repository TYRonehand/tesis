...
public class PlotWidget extends Composite {
	...
	VerticalPanel panel;
	@UiField
	VerticalPanel gridItems;
	// Grid option
	@UiField
	SimpleCheckBox linesGrid;
	@UiField
	SimpleCheckBox pointsGrid;
	// plot live
	@UiField
	Button plotBtn;
	// save
	...
	@UiField
	SimpleCheckBox dimensionCheckbox;
	@UiField
	DoubleBox minxBox;
	@UiField
	DoubleBox maxxBox;
	...
	private void LoadFilesData() {
		serverService.getUserFilesPlot(this.IDUSER,
	...
	private void SaveImg(String extension) {
		...
		if (!this.dimensionCheckbox.getValue()) {
			// service draw
			serverService.CreateImage2D(this.pointsGrid.getValue(),
					this.linesGrid.getValue(), extension,
					this.titleBox.getText(), labelxyz, limitxyz, this.IDUSER,

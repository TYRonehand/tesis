...
public class UserProfile extends Composite {
	...
	//change email
	@UiField PasswordTextBox currentPassword1;
	@UiField TextBox newEmail;
	@UiField TextBox newEmailRepeat;
	@UiField Button changeEmailBTN;
	//change password
	@UiField PasswordTextBox currentPassword2;
	@UiField PasswordTextBox newPassword;
	@UiField PasswordTextBox newPasswordRepeat;
	@UiField Button changePasswordBTN;
	//close account
	@UiField Button closeAccountBTN;
	...
	/*NEW EMAIL*/
	//new email
	private void userNewEmail(String iduser, final String currentPass, final String newEmail, final String repeatEmail){
		//verify user acount
		if(currentPassword1.getText().length()>=6)
		{
			//current password is valid?
			serverService.authenticateUser(UserInfo.getMail(), currentPass, new AsyncCallback<User>(){
				@Override
				public void onFailure(Throwable caught) {
					ErrorVerify.getErrorAlert("offline");
				}
	...
	private void verifyUserEmail(final User myuser){
		//email valid format
		if(newEmail.getText().length()>=6 && FieldVerifier.isValidMail(newEmail.getText()))
		{
			serverService.userExist(newEmail.getText(), new AsyncCallback<Boolean>(){
				...
				public void onSuccess(Boolean result) {
					...
					{
						//equals emails
						if(newEmail.getText().equals(newEmailRepeat.getText()))
						{
							User aux = new User( 
									UserInfo.getId() ,
									newEmail.getText() , 
	...
	/*NEW PASSWORD*/
	private void userNewPassword(String iduser, final String currentPass, final String newpass, final String repeatPass){
		//verify user acount
								...
								if(newPassword.getText().length()>=6)
								{
									...
									if(newPassword.getText().equals(newPasswordRepeat.getText()))
									{
										User aux = new User( 
												...
												result.getOcupation() , 
												result.getWeb() , 
												result.getUniversity(),
												newPassword.getText(),
												result.getCreation(),
									 			result.getRegistered(),
									 			result.getBanned(),
									 			""
									 		  );
										//connect to servlet and send user
										...
										ErrorVerify.getErrorAlert("diferentpassword");
									}
								}else{
									ErrorVerify.getErrorAlert("invalidfield");
								}
	...
	/*DELETE USER INFO*/
	private void userDelete(String iduser)
	{
		if(Window.confirm("Delete your account, are you sure?"))
		{
	...
	void onChangeEmailBTNClick(ClickEvent event){
		userNewEmail(this.UserInfo.getId(), currentPassword1.getText(), newEmail.getText(), newEmailRepeat.getText());
	}
	
	//change user pass
	@UiHandler("changePasswordBTN")
	void onChangePasswordBTNClick(ClickEvent event){
		userNewPassword(this.UserInfo.getId(), this.currentPassword2.getText(), this.newPassword.getText(), this.newPasswordRepeat.getText());
	}
	
	//delete user info
	@UiHandler("closeAccountBTN")
	void onCloseAccountBTNClick(ClickEvent event){
		userDelete(this.UserInfo.getId());
	}

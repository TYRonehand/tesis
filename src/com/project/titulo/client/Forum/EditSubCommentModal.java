...
public class EditSubCommentModal extends DialogBox {
	...
	// topic full
	private SubAnswer updatedAnswer;
	...
	interface EditResponseModalUiBinder extends
			UiBinder<Widget, EditSubCommentModal> {
	}

	public EditSubCommentModal(SubAnswer mycomment) {
	...
	private void EditComment() {
		...
		if (descriptionInput.getText().length() > 3) {
			...
			// call service update
			serverService.setSubComment(updatedAnswer,
					new AsyncCallback<Boolean>() {
						...
						public void onSuccess(Boolean result) {
							if (result) {
								...
								url.GoTo("TOPIC",updatedAnswer.getIduser(),null);
								dialogBox.hide();
							} else {
